...
export default function ManagementAccountsTypeLayout() {
  const {
    loading,
    open,
    setOpen,
          ...
          <Table className="min-w-[600px]">
            ...
            <TableBody>
              {paginatedType.length === 0 ? (
                ...
                paginatedType.map((typeItem, idx) => (
                  <TableRow
                    key={typeItem.id}
                  ...
                  >
                    ...
                    <TableCell className="py-3 px-4 text-xs text-muted-foreground">
                      {FormatIndoDate(typeItem.createdAt)}
                    </TableCell>
                    <TableCell className="py-3 px-4 flex gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="destructive"
                        onClick={() => handleDelete(typeItem.id, typeItem.type)}
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="max-w-md w-full rounded-2xl p-6 bg-background border border-border">
          ...
          <p className="mb-4 text-foreground">
            Apakah Anda yakin ingin menghapus Type <b>{deleteTypeName}</b>?
          </p>
          <DialogFooter className="flex gap-2 justify-end">
            <Button
              variant="destructive"
