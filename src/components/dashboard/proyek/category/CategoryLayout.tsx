...
import useManagementCategory from './lib/useManagementCategory'
...
export default function CategoryLayout() {
    const {
        loading,
        open, setOpen, categoryName, setCategoryName,
        editOpen, setEditOpen, editCategoryName, setEditCategoryName,
        deleteOpen, setDeleteOpen, deleteCategoryName,
        handleSubmit, handleEdit, handleEditSubmit, handleDelete, handleDeleteConfirm,
        // pagination
        currentPage, setCurrentPage, itemsPerPage, totalPages, paginatedCategories
    } = useManagementCategory()
    ...
    return (
        <section>
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 sm:p-8 border rounded-2xl border-border bg-card shadow-sm">
                ...
                <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent className="max-w-md w-full rounded-2xl p-6 bg-background border border-border">
                        <DialogHeader>
                            <DialogTitle className="text-lg font-semibold text-foreground">Add Category</DialogTitle>
                        </DialogHeader>
                        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-2">
                            ...
                            <DialogFooter className="flex gap-2 justify-end">
                                <Button type="submit" disabled={loading || !categoryName} className="bg-primary hover:bg-primary/90 text-primary-foreground font-medium px-4 py-2 rounded-lg transition-colors">
                                    {loading ? 'Adding...' : 'Add'}
                                </Button>
                                <DialogClose asChild>
                    ...
                    <Table className="min-w-[600px]">
                        <TableHeader>
                            <TableRow className="bg-muted">
                                ...
                                <TableHead className="py-3 px-4 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">Nama Kategori</TableHead>
                        ...
                        <TableBody>
                            {paginatedCategories.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={4} className="text-center text-muted-foreground py-6">No categories yet.</TableCell>
                                </TableRow>
                            ) : (
            ...
            <Dialog open={editOpen} onOpenChange={setEditOpen}>
                <DialogContent className="max-w-md w-full rounded-2xl p-6 bg-background border border-border">
                    <DialogHeader>
                        <DialogTitle className="text-lg font-semibold text-foreground">Edit Category</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleEditSubmit} className="flex flex-col gap-4 mt-2">
                        ...
                        />
                        <DialogFooter className="flex gap-2 justify-end">
                            <Button type="submit" disabled={loading || !editCategoryName} className="bg-primary hover:bg-primary/90 text-primary-foreground font-medium px-4 py-2 rounded-lg transition-colors">
                                {loading ? 'Saving...' : 'Save'}
                            </Button>
                            <DialogClose asChild>
            ...
            <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                <DialogContent className="max-w-md w-full rounded-2xl p-6 bg-background border border-border">
                    <DialogHeader>
                        <DialogTitle className="text-lg font-semibold text-foreground">Hapus Kategori</DialogTitle>
                    </DialogHeader>
                    <p className="mb-4 text-foreground">Apakah Anda yakin ingin menghapus kategori <b>{deleteCategoryName}</b>?</p>
                    <DialogFooter className="flex gap-2 justify-end">
